...
local function mismatchFormattingPureList(table_a, table_b, margin)
    ...
    * result: if success is true, a multi-line string with deep analysis of the two lists
    ...
    for i = 0, shortest - 1 do
        if
            not M.private.is_table_equals(table_a[len_a - i], table_b[len_b - i], margin)
        then
            commonBackTo = i - 1
            break
        end
...
function M.almostEquals(actual, expected, margin)
    ...
    if margin < 0 then
        error_fmt(
            3,
            "almostEquals: margin must not be negative, current value is " .. margin
        )
    end
...
function JUnitOutput:endSuite()
    ...
    self.fd:write(
        string.format('            <property name="Lua Version" value="%s"/>\n', _VERSION)
    )
    self.fd:write(
        string.format(
...
function M.LuaUnit:execOneFunction(className, methodName, classInstance, methodInstance)
    ...
    for iter_n = 1, self.exeRepeat or 1 do
        ...
        if classInstance then
            ...
            if func then
                self:updateStatus(
                    self:protectedCall(classInstance, func, className .. ".setUp")
                )
            end
        end
        ...
        if classInstance then
            ...
            if func then
                self:updateStatus(
                    self:protectedCall(classInstance, func, className .. ".tearDown")
                )
            end
        end
    end
...
function M.LuaUnit:setupClass(className, instance)
    if type(instance) == "table" and self.asFunction(instance.setupClass) then
        self:updateStatus(
            self:protectedCall(instance, instance.setupClass, className .. ".setupClass")
        )
    end
end
