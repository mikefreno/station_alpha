...
local function handleController()
    ...
    if
        currentTime - overlayStats.lastControllerCheck < overlayStats.CONTROLLER_COOLDOWN
    then
        return
    end
...
local function toggleVSync()
    ...
    print(
        string.format("VSync %s", overlayStats.vsyncEnabled and "enabled" or "disabled")
    )
end
...
local function isTouchInsideOverlay(x, y)
    ...
    return x >= area.x
        and x <= area.x + area.width
        and y >= area.y
        and y <= area.y + area.height
end
...
function overlayStats.draw()
    ...
    local versionTextWidth =
        font:getWidth(string.format("%s", overlayStats.renderInfo.version))
    local rendererInfoWidth = font:getWidth(
        string.format(
            "Renderer: %s (%s)",
            overlayStats.renderInfo.name,
            overlayStats.renderInfo.vendor
        )
    )
    local systemInfoWidth = font:getWidth(
        overlayStats.sysInfo.os
            .. " "
    ...
    local contentWidth =
        math.max(versionTextWidth, rendererInfoWidth, systemInfoWidth, baseWidth)
    ...
    love.graphics.print(
        string.format("Canvas Switches: %d", currentCanvasSwitches),
        20,
        y
    )
    y = y + 20
    ...
    love.graphics.print(
        string.format("Shader Switches: %d", currentShaderSwitches),
        20,
        y
    )
    y = y + 20
    ...
    love.graphics.print(
        string.format(
            "Draw Calls: %d (%d batched)",
            currentDrawCalls,
            currentDrawCallsBatched
        ),
        20,
        y
    )
    ...
    love.graphics.print(
        string.format("Particles: %d", math.floor(currentParticleCount)),
        20,
        y
    )
    y = y + 20
    ...
    love.graphics.setColor(
        overlayStats.supportedFeatures.glsl3 and { 0, 1, 0, 1 } or { 1, 0, 0, 1 }
    )
    love.graphics.print(
        string.format(
            "GLSL 3: %s",
            overlayStats.supportedFeatures.glsl3 and "Yes" or "No"
        ),
        20,
        y
    )
    ...
    love.graphics.setColor(
        overlayStats.supportedFeatures.pixelShaderHighp and { 0, 1, 0, 1 }
            or { 1, 0, 0, 1 }
    )
    love.graphics.print(
        string.format(
            "Pixel Shader highp: %s",
            overlayStats.supportedFeatures.pixelShaderHighp and "Yes" or "No"
        ),
        20,
        y
    )
    ...
    love.graphics.print(
        string.format("VSync: %s", overlayStats.vsyncEnabled and "ON" or "OFF"),
        20,
        y
    )
...
function overlayStats.update(dt)
    ...
    overlayStats.metrics.drawCallsBatched[overlayStats.currentSample] =
    ...
    overlayStats.metrics.textureMemory[overlayStats.currentSample] = stats.texturememory
        / (1024 * 1024)
