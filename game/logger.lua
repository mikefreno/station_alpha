...
function Logger:_calculate_total_content_height()
  ...
  for i = 1, #self.logs do
    ...
    local full_text_message = string.format("[%s][%s] %s", log_item.timestamp, log_item.level.name, log_item.message)
...
function Logger:_copy_to_clipboard()
  ...
  local text_to_copy = table.concat(clipboard_text, "\n")

  -- Try to copy to clipboard using love.system.setClipboardText if available
  if love.system and love.system.setClipboardText then
    ...
    print("Copied to clipboard (fallback):")
    print(text_to_copy)
  end

  -- Optionally show a visual confirmation
  self:info("Logs copied to clipboard")
end
